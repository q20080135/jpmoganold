...
(function () {
    ...
    OnlineImage.prototype = {
        init: function () {
            this.dl_config = {         //文件夹列表options
                animation_delay : 300       //开关动作时间
                ,animation_status : true    //文件夹开关动画完成状态
                ,dir_status : true     //文件夹列表打开状态
                ,wrapper_width : 0
                ,init_complete : false
                ,wrapper : "#dirList"
            };
        ...
        initDirectory: function () { 
            ...
            var _this = this;
            $(this.dl_config.wrapper).on('mouseover',function(){
                if(_this.dl_config.init_complete){
                    _this.show_dir_list();
                }
            });
            ...
            $(this.dl_config.wrapper).show();

            this.dl_config.wrapper_width = $(this.dl_config.wrapper).width();
            setTimeout(function(){
                _this.dl_config.init_complete = true;
                _this.hide_dir_list();
            },1000); 
        },
        show_dir_list : function(){
            if(this.dl_config.animation_status && !this.dl_config.dir_status){
                this.dl_config.animation_status = false;
                $("#dirList").animate({left:"0px"},this.dl_config.animation_delay,'swing');
                setTimeout(this.set_status(),this.dl_config.animation_delay+300);
                this.dl_config.dir_status = true;
            }
        },
        set_status : function(){
            this.dl_config.animation_status = true;
        },
        hide_dir_list: function (){ 
            if(this.dl_config.animation_status && this.dl_config.dir_status){
                this.dl_config.animation_status = false;
                $(this.dl_config.wrapper).animate({left:"-"+this.dl_config.wrapper_width+"px"},this.dl_config.animation_delay,'swing');
                setTimeout(this.set_status(),this.dl_config.animation_delay+300);
                this.dl_config.dir_status = false;
            }
        },
        /* 向后台拉取图片列表数据 */
        renderDirData: function () {
            if(!this.dl_config.init_complete){
                this.initDirectory();
            }
            var list = this.dirListData;
            var html = '<ul class="list">';
            var var_class = '';
            for (i = 0; i < list.length; i++) {
                if(list[i] == this.select_dir) {
                    var_class =' class="selected"';
                }else{
                    var_class = '';
                }
                html += '<li '+var_class+'>'+list[i]+'</li>';
            }
            html += '</ul>';
            $(this.dl_config.wrapper).html(html);

            var _this = this;
            $(this.dl_config.wrapper+' .list li').on('click',function(){
                _this.select_dir = $(this).text();
                _this.reset();
            });
        },
        /* 初始化滚动事件,滚动到地步自动拉取数据 */
                    ...
                    'onsuccess': function (r) {
                        try {
                            ...
                            if (json.state == 'SUCCESS') {
                                ...
                                _this.dirListData = json.dir;
                                _this.select_dir = json.select_dir;
