...
function auto_load_tab(){

	// var v = getCookie("last_tab");
	// Hui_admin_tab(v);
	var _href = window.location.hash;
	var _href = _href.replace('#','');
	// var reg=/([^\s]+(?=\.(php|htm|html))\.\2)/gi;
	var reg=/(\/jt_admin[^\s]+)/ig;

	if(reg.test(_href)){
		var _titleName = getCookie("last_tab_title");
...
function Hui_admin_tab(obj){
	if($(obj).attr('_href')){
		...
		if(_titleName == undefined){
			_titleName = $(obj).text();
		}
		open_new_tab(_href,_titleName);
	}
}
function open_new_tab(href,title){
	...
	if(!(href == undefined || href == '')){
		...
		setCookie("last_tab_title", title);
...
$(function(){
	...
	$(document).on("click","#min_title_list li",function(){
		...
		setCookie("last_tab_title", $("#min_title_list li.active").text());
		window.location.hash = $("#min_title_list li.active span").attr('data-href');
