...
class Common_util_pub
{
	 ...
	 * 	作用：使用证书，以post方式提交xml到对应的接口url
	 */
	function postXmlSSLCurlApp($xml,$url,$second=30)
	{
		$ch = curl_init();
		//超时时间
		curl_setopt($ch,CURLOPT_TIMEOUT,$second);
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml()
	{
	   	...
	   	//$this->parameters["appid"] = "wxf3fbd509fb3c8918";  
		//$this->parameters["mch_id"] = "1324601001";
	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["out_trade_no"] == null) 
			{
		   	...
		   	$this->parameters["spbill_create_ip"] = $this->getIp();//$_SERVER['REMOTE_ADDR'];//终端ip	    
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
		    return  $this->arrayToXml($this->parameters);
...
class JsApi_pub extends Common_util_pub
{
	 ...
	 * 	作用：生成可以获得code的url
	 */
	function createOauthUrlForCode2($redirectUrl)
	{
		$urlObj["appid"] = APPID;
		$urlObj["redirect_uri"] = urlencode($redirectUrl);
		$urlObj["response_type"] = "code";
	...
	function getOpenid()
	{
		...
		//运行curl，结果以jason形式返回
        $res = curl_exec($ch);
        //error_log('getopenid data = '.print_r($res,1));
		curl_close($ch);
		//取出openid
	...
	/**
	 * 	作用：通过curl向微信提交code，以获取openid  然后获取用户信息
	 */
	 ...
	 * 	作用：设置nonce_str
	 */
	function setNonceStr($nonce_str)
	{
	...
	public function getAppParameters()
	{
		...
		$jsApiObj["timestamp"] = "{$timeStamp}";
		//$jsApiObj["noncestr"] = $this->createNoncestr();
		$jsApiObj["noncestr"] = $this->nonce_str;
	...
	public function getWebParameters()
	{
		$timeStamp = time();
		$jsApiObj["appid"] = APPID;
		$jsApiObj['mch_id'] = MCHID;
		$jsApiObj["timestamp"] = "{$timeStamp}";
		$jsApiObj["nonce_str"] = $this->nonce_str;
		$jsApiObj["package"] = "$timeStamp";
		$jsApiObj["prepayid"] = $this->prepay_id;
		$jsApiObj["sign"] = $this->getSign($jsApiObj);
		$this->parameters = json_encode($jsApiObj);
	}
