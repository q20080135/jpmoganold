...
class AopClient {
	...
	public function sdkExecute($request) {
		...
		//PHP5.3 http_build_query 不支持将空格转换成%20，用以下内容替换
		$encodeStr = '';
		foreach ($params as $key => &$value) {
			if ($key == 'timestamp') {
				$encodeStr .= "&" . "$key" . "=" . rawurlencode($value);
			} else {
				$encodeStr .= "&" . "$key" . "=" . urlencode($value);
			}
		}
		return substr($encodeStr, 1);
	}
